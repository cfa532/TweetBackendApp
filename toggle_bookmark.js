 ...
 * - Requires the tweet storage owner's root node and addresses the user's root node
 * - Syncs content when bookmarked
 * - Reports failures from either write; the two writes are not atomic
 * 
...
((request, args)=>{
    ...
    try {
        ...
        if (author.hostIds[0] !== nodeId) {
            throw new Error("Bookmark mutation must run on the tweet storage owner's root node")
        }
        ...
        let updatedUser
        try {
            const userResp = lapi.RunMApp("toggle_bookmark_by_user", {aid: APP_ID, ver: "last",
                nid: userHostId, sid: systemSid, version: "v2",
                userid: userId, tweetid: tweetId, isbookmarked: updatedTweet.favorites[1],
                skipcontentsync: userHostId === nodeId}, []
            )
            // A remote entry reports failures in its response envelope.
            if (!userResp?.success || !userResp.data) {
                throw new Error(userResp?.message || "Failed to update user bookmarks")
            }
     ...
     * @returns {Object} Updated tweet object; throws on failure
     */
    function updateBookmarkOfTweet(
        ...
        try {
            ...
            if (!tweetResp?.success || !tweetResp.data) {
                throw new Error(tweetResp?.message || "Failed to read updated tweet")
            }
