...
The Go backend, as a Leither MApp. The superseded JavaScript implementation is
kept for reference in `legacy-js/`. It serves Tweet-iOS, Tweet for Android and TweetWeb.
...
App IDs, admin nodes and the browser-fallback domain checklist are in
`docs/ENVIRONMENTS_AND_CLIENT_NOTES.md`.
...
rsync -av -e 'ssh -p 220' \
  ...
  --include='*.go' \
  --exclude='*' \
  ./ pi@gen8.leither.uk:/home/pi/demo/twbe/
```
