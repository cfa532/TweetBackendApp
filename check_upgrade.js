...
((request, args) => {
    ...
    try {
         ...
         * - Package ID: 9OCLYP-SXzen3e171-Ei_6N3Gwl        # p5-_uQPHjQpWI7gD4Zw65LrdRPm
         * - Command line tool: ./release1_upgrade.sh app-release.apk
         */
        let ret = {
            // Dynamic version from package: appVersion.Versions[appVersion.Versions.length-1].Version
            version: 74,  // Set larger than defaultConfig.versionName in build.gradle to force upgrade
            ...
            domain: "t1.ghrwwregas.site",  // Base URL for deeplinks and sharing
        }
        ...
        lapi.Error("Tweed Error check_upgrade: %s, request=%s", e, JSON.stringify(request))
        return wrapError(e)
    }
})(request, args)
